...
const IconTrash = (props) => (
  ...
  >
    ...
    <path
      d="M5.5 8h13l-1 12.5a1.5 1.5 0 0 1-1.5 1.4h-8a1.5 1.5 0 0 1-1.5-1.4L5.5 8Zm4 2.2v9m5-9v9"
      stroke="#fff"
      strokeWidth="1.1"
    />
  </svg>
...
export default function SetShippingCostPage() {
  ...
  return (
    <div className="min-h-screen bg-[#eef2f9] px-4 py-6 sm:px-8 sm:py-6 font-sans text-[#3b4256]">
      {/* Page header */}
      <div className="mb-4">
        <h1 className="text-[28px] font-semibold text-[#232f4b] leading-tight">
          Set Shipping Cost
        </h1>
      ...
      <div className="rounded-[10px] bg-white px-5 py-6 shadow-[0_2px_10px_rgba(30,42,80,0.06)] sm:px-7">
        <div className="flex items-center justify-between">
          <h2 className="m-0 text-xl font-semibold text-[#232f4b]">
            Set Shipping Cost
          </h2>
          <button
            onClick={() => router.push("/admin/set-shipping/add")}
            className="rounded-md bg-[#2f55d4] px-[18px] py-2 text-sm font-medium text-white hover:bg-[#2645b0]"
          >
            Add Cost
          </button>
        </div>
          ...
          <table className="w-full border-collapse text-[14.5px]">
            ...
            <tbody>
              {pageRows.length === 0 ? (
                ...
                pageRows.map((r, i) => (
                  <tr key={r.id}>
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top text-[#2f55d4]">
                      {(currentPage - 1) * perPage + i + 1}
                    </td>
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top font-medium text-[#232f4b]">
                      {r.province}
                    </td>
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top text-[#4b5468]">
                      {r.city}
                    </td>
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top text-[#4b5468]">
                      {r.cost.toFixed(2)}
                    ...
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top">
                      <button
                        onClick={() => toggleShipping(r.id)}
                        className={`relative inline-flex h-[22px] w-[42px] items-center rounded-full transition-colors ${r.applyShipping ? "bg-[#2f55d4]" : "bg-[#d5d9e3]"}`}
                      >
                        <span
                          className={`inline-block h-[18px] w-[18px] transform rounded-full bg-white shadow transition-transform ${r.applyShipping ? "translate-x-[22px]" : "translate-x-[2px]"}`}
                        />
                      </button>
                    </td>
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top text-[#4b5468]">
                    ...
                    <td className="border-b border-[#f1f2f6] px-2 py-3 align-top">
                      <div className="flex gap-2">
                        ...
                        <button
                          title="Edit"
                          onClick={() => openEdit(r)}
                          className="flex h-[30px] w-[30px] items-center justify-center rounded-md border border-[#d6ddf7] bg-[#eef1fd] text-[#2f55d4] hover:brightness-95"
                        >
                          <IconEdit />
                        </button>
                        <button
        ...
        <div className="mt-4.5 flex flex-wrap items-center justify-between gap-2.5 text-[13.5px] text-[#6b7285]">
          ...
          <div className="flex gap-1">
            <button
              ...
              className="rounded-[5px] border border-[#e1e4eb] bg-[#f4f6fa] px-3 py-1.5 text-[13.5px] text-[#4b5468] disabled:cursor-not-allowed disabled:opacity-50"
            >
              Previous
            </button>
            {pageNumbers.map((n) => (
              <button
                key={n}
                onClick={() => setPage(n)}
                className={`rounded-[5px] border px-3 py-1.5 text-[13.5px] ${n === currentPage ? "border-[#2f55d4] bg-[#2f55d4] text-white" : "border-[#e1e4eb] bg-[#f4f6fa] text-[#4b5468]"}`}
              >
                {n}
              </button>
            ))}
      ...
      {showTop && (
        <button
          ...
          className="fixed bottom-6 right-6 flex h-[42px] w-[42px] items-center justify-center rounded-full bg-[#2f55d4] text-white shadow-[0_4px_12px_rgba(47,85,212,0.4)]"
        >
          <IconArrowUp />
        </button>
      )}
      ...
      {showModal && (
        ...
        >
          <div
            ...
            className="w-[400px] max-w-[90vw] rounded-[10px] bg-white p-6 shadow-[0_10px_40px_rgba(0,0,0,0.2)]"
          >
            <h3 className="mb-4 text-lg text-[#232f4b]">
              {editing ? "Edit Shipping Cost" : "Add Shipping Cost"}
            </h3>

            <label className="mb-1.5 block text-[13px] text-[#6b7285]">
              Province
            </label>
            <input
              type="text"
              ...
              className="mb-3 w-full rounded-md border border-[#dfe3ea] px-2.5 py-2 text-sm focus:border-[#2f55d4] focus:outline-none"
            />
            ...
            <input
              ...
              placeholder="0.00"
              className="mb-1 w-full rounded-md border border-[#dfe3ea] px-2.5 py-2 text-sm focus:border-[#2f55d4] focus:outline-none"
            />

            <div className="mt-5 flex justify-end gap-2.5">
              <button
                onClick={() => setShowModal(false)}
                className="rounded-md bg-[#f1f2f6] px-4 py-2 text-sm text-[#4b5468]"
              >
                Cancel
              </button>
              <button
