...
import { LayoutDashboard, Info, Edit2 } from "lucide-react";
import { toast } from "react-toastify";

const CARRIERS_API = "/api/shipping-carriers";
...
export default function CarriersList() {
  ...
  const handleTogglePublish = async (id, currentStatus) => {
    try {
      const res = await fetch(`${CARRIERS_API}/${id}`, {
        method: "PATCH",
      ...
      if (data.success) {
        setCarriers((prev) =>
          prev.map((c) => (c.id === id ? { ...c, publish: !currentStatus } : c))
        );
      }
    } catch (err) {
  ...
  const handleInfo = (carrier) => {
    toast.info(`${carrier.name} | ${carrier.address || "No address"} | ${carrier.phone || "No phone"} | ${carrier.type}`);
  };

  const handleEdit = (id) => {
    router.push(`/admin/shipping-carriers/edit/${id}`);
  };
  ...
  return (
    <div className="h-full bg-gray-100 flex flex-col">
      <div className="flex-1 p-6">
        ...
        <div className="mb-4">
          <h1 className="text-2xl font-normal text-gray-900">Carriers</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="px-6 py-5 border-b border-gray-200">
            <h2 className="text-lg font-bold text-blue-900">Carriers</h2>
          </div>
                        ...
                        {/* <td className="px-3 py-4 border-r border-gray-200">
                          <button
                            ...
                            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                              carrier.publish ? "bg-blue-600" : "bg-gray-300"
                            }`}
                          >
                            <span
                              className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
                                carrier.publish ? "translate-x-6" : "translate-x-1"
                              }`}
                            />
                          </button>
                        </td> */}
