...
export default function EditCarrier() {
  ...
  useEffect(() => {
    async function fetchCarrier() {
      try {
        const res = await fetch(`/api/shipping-carriers/${id}`);
        const data = await res.json();
        if (!res.ok || !data?.success) {
          throw new Error(data?.message || "Failed to load carrier.");
        }
        setForm({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(data.message || "Carrier updated successfully.");
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <div className="flex-1 p-3 sm:p-6">
        {/* Breadcrumb */}
        <div className="mb-4">
          <h1 className="text-xl sm:text-2xl font-normal text-gray-900">Edit Carrier</h1>
          <p className="flex items-center gap-1 text-sm text-gray-500 mt-1">
            ...
            <span>/</span>
            <Link href="/admin/shipping-carriers/carrier-list" className="hover:underline text-gray-500">
              Carriers
            </Link>
            <span>/</span>
            <span>Edit</span>
        ...
        <div className="bg-white rounded-xl shadow-sm w-full px-6 sm:px-10 py-10">
          <h2 className="text-xl font-medium text-blue-900 text-center mb-8">Edit Carrier</h2>
          ...
          {loading ? (
            ...
            <>
              ...
              {message ? <p className="mt-4 text-center text-sm text-green-700">{message}</p> : null}
            </>
          )}
        </div>
      </div>
