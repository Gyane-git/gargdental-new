...
export default function DashboardPage() {
    ...
    const fetchAll = async () => {
      try {
        const [bannersRes, productsRes, brandsRes, categoriesRes, customersRes, inquiriesRes] = await Promise.allSettled([
          fetch("/api/v1/banners?include_inactive=1").then((r) => r.json()),
          fetch("/api/v1/products?include_inactive=1").then((r) => r.json()),
          fetch("/api/v1/brands?include_inactive=1").then((r) => r.json()),
          fetch("/api/v1/categories?flat=1").then((r) => r.json()),
  ...
  const getBannerImage = (b) => {
    ...
    if (b.mobile_image_full_url) return b.mobile_image_full_url;
    return "/assets/logo.png";
  };

  const getProductImage = (p) => {
    if (p.main_image_full_url) return p.main_image_full_url;
    return "/no-image.png";
  };
          ...
          <RecentTable title="Recent Products" href="/admin/products" loading={loading} empty={recentProducts.length === 0} emptyText="No products yet" router={router}>
            {recentProducts.map((p) => (
              <tr key={p.id} className="border-b last:border-0 hover:bg-slate-50 transition-colors">
                <td className="px-5 py-3">
                  <div className="w-10 h-10 rounded-lg overflow-hidden border border-gray-100 bg-gray-50">
                    <img src={getProductImage(p)} alt={p.product_name} className="w-full h-full object-cover" />
                  </div>
                </td>
                <td className="px-3 py-3">
                  <p className="text-sm font-semibold text-gray-800 truncate max-w-[160px]">{p.product_name}</p>
                  <p className="text-xs text-gray-400 font-mono">#{p.id}</p>
                </td>
                <td className="px-3 py-3">
                  <p className="text-sm font-semibold text-[#1a3a6b]">{p.sell_price ? `Rs. ${Number(p.sell_price).toLocaleString()}` : "—"}</p>
                </td>
                <td className="px-3 py-3">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {loading ? (
              ...
              <p className="text-center text-sm text-gray-400 py-8">No inquiries yet</p>
            ) : (
              recentInquiries.map((i) => (
                <div key={i.id} className="flex items-center gap-3 px-5 py-3 hover:bg-slate-50 transition-colors">
                  {/* Avatar initials */}
                  <div className="w-8 h-8 rounded-full bg-[#1a3a6b] flex items-center justify-center flex-shrink-0">
                    <span className="text-xs font-bold text-white">{(i.name || "?").charAt(0).toUpperCase()}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">{i.name || "Unknown"}</p>
                    <p className="text-xs text-gray-400 truncate">{i.subject || i.email || "—"}</p>
                  </div>
                </div>
              ))
            )}
