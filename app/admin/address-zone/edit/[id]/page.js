...
export default function EditAddressZonePage() {
  ...
  const fetchZone = async () => {
    ...
    try {
      setLoadingZone(true);
      const res = await fetch(`/api/v1/addresses/address-zone/${zoneId}`, {
        cache: "no-store",
      });
      const data = await res.json();
      ...
      if (data.success && data.zone) {
        setForm({
          city_id: data.zone.city_id ? String(data.zone.city_id) : "",
          zone_name: data.zone.zone_name || "",
        });
      } else {
        toast.error(data.message || "Failed to load zone");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(data.message || "Zone updated successfully.");
      toast.success(data.message || "Zone updated successfully.");
      setTimeout(() => {
  ...
  const loading = loadingCities || loadingZone;
    ...
    <div className="flex min-h-screen flex-col bg-[#eef2f9] font-sans text-[#3b4256]">
      <div className="flex flex-1 items-start justify-center px-4 py-12 sm:py-16">
        <form onSubmit={handleSubmit} className="w-full max-w-[500px] rounded-[10px] bg-white p-8 shadow-[0_2px_10px_rgba(30,42,80,0.06)] sm:p-10">
          <h1 className="mb-8 text-center text-2xl font-semibold text-[#232f4b]">Edit Address Zone</h1>
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-md border border-[#e1e4eb] bg-white px-3 py-2.5 text-[15px] text-[#3b4256] focus:border-[#2f55d4] focus:outline-none disabled:bg-[#f6f7fa]"
                />
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 flex items-center justify-between">
            <button
              ...
              className="rounded-md bg-[#1f8a5f] px-6 py-2.5 text-[15px] font-medium text-white hover:bg-[#1a7550] disabled:cursor-not-allowed disabled:opacity-60"
            >
