...
const isDefaultFilters = (filters) => Object.keys(DEFAULT_FILTERS).every((key) => filters[key] === DEFAULT_FILTERS[key]);
...
function ViewChangesModal({ log, onClose, actionBadgeStyles }) {
  useEffect(() => {
    const onKeyDown = (e) => {
  ...
  const oldData = parseDataField(log.oldData ?? log.old_data);
  const newData = parseDataField(log.newData ?? log.new_data);
  const oldDataEntries = oldData ? Object.entries(oldData) : null;
  const newDataEntries = newData ? Object.entries(newData) : null;

  const modelLabel = log.model || MODULE_LABELS[log.module] || log.module;
  const modelId = log.modelId ?? log.model_id;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b bg-gray-50 rounded-t-lg">
          <h3 className="text-lg font-semibold text-gray-900">Activity Details #{log.id}</h3>
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-5">
          <div className="bg-gray-50 rounded-lg p-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <p className="text-xs font-semibold text-gray-500 mb-1">Action</p>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-500 mb-1">Model</p>
            ...
            <div>
              ...
              <p className="text-sm text-gray-800">{MODULE_LABELS[log.module] || log.module}</p>
            </div>

            <div>
              <p className="text-xs font-semibold text-gray-500 mb-1">Admin</p>
              <p className="text-sm text-gray-800">
                {log.admin}
                {log.role ? ` (${log.role})` : ""}
              </p>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-500 mb-1">IP Address</p>
              <p className="text-sm text-pink-500 font-mono">{log.ip}</p>
            </div>
            <div>
            ...
            {log.userAgent || log.user_agent ? (
              <div className="sm:col-span-3">
                ...
                <p className="text-sm text-gray-700 break-words">{log.userAgent || log.user_agent}</p>
              </div>
            ) : null}
          </div>
          ...
          {oldDataEntries ? (
            <div>
              <h4 className="flex items-center gap-1 text-red-600 font-semibold text-sm mb-2">
                <span aria-hidden>→</span> Old Data
              </h4>
              <DataTable entries={oldDataEntries} />
            </div>
          ) : null}

          {newDataEntries ? (
            <div>
              <h4 className="flex items-center gap-1 text-green-600 font-semibold text-sm mb-2">
                <span aria-hidden>→</span> New Data (Complete Record)
              </h4>
              <DataTable entries={newDataEntries} />
            </div>
          ) : null}

          {!oldDataEntries && !newDataEntries ? <p className="text-sm text-gray-400 text-center py-6">No additional change details available for this entry.</p> : null}
        </div>

        <div className="px-6 py-4 border-t flex justify-end">
          <button onClick={onClose} className="px-5 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50">
            Close
          </button>
        </div>
      </div>
...
export default function AuditLogsPage() {
  ...
  useEffect(() => {
    ...
    const loadLogs = async () => {
      ...
      try {
        ...
        const [logsResponse, groupsResponse] = await Promise.all([fetch(`/api/v1/admin/audit-logs?${query}`, { cache: "no-store" }), shouldLoadMeta ? fetch("/api/system-users/groups", { cache: "no-store" }) : Promise.resolve(null)]);
  ...
  const closeViewChanges = () => setSelectedLog(null);
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {dateShortcuts.map((shortcut) => (
                <button key={shortcut.label} onClick={shortcut.onClick} className="px-3 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-50 text-gray-700">
                  {shortcut.label}
                </button>
              ))}
        ...
        <div className="bg-white rounded-lg shadow-sm mt-6 overflow-hidden">
          ...
          <div className="px-5 sm:px-6 py-4 flex items-center justify-between flex-wrap gap-3 border-t">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={currentPage === 1} className="px-3 py-1.5 text-sm border rounded disabled:opacity-40">
                Previous
              </button>
              ...
              <button onClick={() => setCurrentPage((page) => Math.min(totalPages, page + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 text-sm border rounded disabled:opacity-40">
                Next
              </button>
            </div>
          </div>
      ...
      {selectedLog ? <ViewChangesModal log={selectedLog} onClose={closeViewChanges} actionBadgeStyles={actionBadgeStyles} /> : null}
