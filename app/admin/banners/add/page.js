...
export default function AddBannerPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        // /api/v1/products/all is the mobile catalog-browse endpoint (full serialization incl.
        // variations/reviews per row, one product per active top-level row) and doesn't support
        // include_inactive at all - this picker only needs product_code/product_name, so it uses
        // the admin-only, already-optimized /api/v1/products list instead.
