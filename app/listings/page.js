...
const DentalSuppliesListing = () => {
  ...
  const filteredProducts = products.filter((product) => {
    const matchesSearch = product.product_name.toLowerCase().includes(searchTerm.toLowerCase()) || product.product_code.toLowerCase().includes(searchTerm.toLowerCase()) || product.brand.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const filteredAndSortedProducts = useMemo(() => {
    let filtered = products.filter((product) => {
      ...
      const matchesCategory = !filters.category || (categoryMap[filters.category] || []).includes(product.category_id);
      ...
      <div className="min-h-100 flex items-center justify-center bg-gray-50">
        <div className="flex flex-col items-center gap-4">
          <div className="flex gap-2">
            ...
            <span className="h-3 w-3 rounded-full bg-orange-600 animate-bounce" style={{ animationDelay: "150ms" }}></span>
    ...
    <>
      <div className="max-w-7xl mx-auto sm:-ml-8 -my-3 sm:-my-4 lg:-my-6 p-2 sm:p-4 lg:p-6 ">
        ...
        <div className="bg-gray-50 p-3 sm:p-4 lg:p-5 rounded-lg mb-3 sm:mb-4 lg:mb-5 shadow">
          <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-blue-900 mb-2 sm:mb-2.5">Browse Supplies Results</h1>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4 gap-2 sm:gap-0">
            <span className="text-gray-600 text-sm sm:text-base">Showing dental supplies and equipment</span>
          </div>
        </div>
        {/* Filters */}
        <div className="mb-6 sm:mb-8  pb-4 sm:pb-6 border-b">
          <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-2 sm:gap-4">
            ...
            <div className="relative w-full sm:w-auto">
              <select value={filters.brand} onChange={(e) => handleFilterChange("brand", e.target.value)} className="appearance-none border border-gray-300 rounded-lg px-3 sm:px-4 py-2 pr-6 sm:pr-8 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 text-sm w-full sm:w-auto">
            ...
            {/* Price Range Filter */}
            <div className="relative w-full sm:w-auto">
              <select value={filters.priceRange} onChange={(e) => handleFilterChange("priceRange", e.target.value)} className="appearance-none border border-gray-300 rounded-lg px-3 sm:px-4 py-2 pr-6 sm:pr-8 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50 text-sm w-full sm:w-auto">
            ...
            {(filters.category || filters.brand || filters.priceRange) && (
              <button onClick={clearFilters} className="text-blue-600 hover:text-blue-800 font-medium text-sm sm:text-base w-full sm:w-auto text-left sm:text-center">
                Clear all filters
              </button>
            )}
          </div>
          ...
          <div className="min-h-100 flex items-center justify-center bg-gray-50">
            <div className="flex flex-col items-center gap-4">
              <div className="flex gap-2">
                ...
                <span className="h-6 w-6 rounded-full bg-orange-600 animate-bounce" style={{ animationDelay: "150ms" }}></span>
                <span className="h-6 w-6 rounded-full bg-indigo-600 animate-bounce" style={{ animationDelay: "300ms" }}></span>
              </div>
          ...
          <div>
            ...
            <div className="max-w-7xl mx-auto px-2 sm:px-4">
              <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 sm-gap-x-6 gap-x-4 gap-y-4 ">
                {filteredAndSortedProducts.slice(0, visibleCount).map((product, index) => (
                  <ProductCardMain key={product.id || index} product={product} showDiscount={parseFloat(product.actual_price) > parseFloat(product.sell_price)} />
                ))}
              </div>
            </div>
            ...
            {filteredAndSortedProducts.length > visibleCount && (
              <div className="flex justify-center mt-6">
                <button onClick={handleLoadMore} className="mt-6 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors cursor-pointer">
                  Load More
                </button>
              </div>
            )}
            ...
            {filteredAndSortedProducts.length === 0 && (
              <div className="text-center py-8 sm:py-12">
                ...
                <button onClick={() => clearFilters} className="mt-3 sm:mt-4 text-blue-600 hover:text-blue-800 font-medium text-sm sm:text-base">
    ...
    <div className="flex flex-col sm-h-[250px] h-full min-h-[340px] bg-gray-50 rounded-lg shadow-md hover:shadow-2xl hover:scale-105 transition-transform duration-300 p-2 sm:p-3 lg:p-4">
      <div className="flex-1 flex flex-col cursor-pointer">
        <div className="relative mb-2 sm:mb-3 lg:mb-4">
          ...
          {showDiscount && product.actual_price && <div className="absolute top-8 sm:top-8 right-1 sm:right-2 bg-red-500 text-white text-xs px-1 sm:px-2 py-0.5 sm:py-0.5 animate-bounce rounded-2xl">OFFER</div>}
        </div>
      ...
      {!product.has_variations && product.available_quantity > 0 && product.stock_quantity > 0 && (
        <>
          <div className="mt-auto w-full">
      ...
      {product.stock_quantity === 0 && product.available_quantity === 0 && !product.has_variations && (
        <button disabled className="w-full py-2 rounded-lg bg-gray-400 text-white font-semibold cursor-not-allowed opacity-80">
          Out of Stock
        </button>
      )}
    </div>
