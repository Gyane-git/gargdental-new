...
// Ports BannerController::get_promotions (BannerController.php:113-134). Promotion.php:33-77.
/**
 * @swagger
 * /api/v1/promotions:
 *   get:
 *     summary: List active promotions, each with its linked product
 *     tags: [Promotions]
 *     responses:
 *       200:
 ...
 *                 message: { type: string, example: "Promotions fetched successfully." }
 *                 promotions:
 *                   type: array
 *                   items:
 *                     type: object
...
export async function GET() {
  try {
    const rows = await prisma.promotion_images.findMany({ where: { status: 1 } });
    ...
    return successResponse("Promotions fetched successfully.", { promotions });
  } catch (error) {
    console.error("Exception occurred while fetching promotions", error);
    return serverErrorResponse("Failed to get promotions", error);
  }
}
