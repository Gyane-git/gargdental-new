...
import { Search, Package, ChevronDown, Tag, Loader2, AlertCircle, RotateCcw } from "lucide-react";
...
const ProductAPIRequest = () => {
  ...
  const { products, loading, loadingMore, error, pagination, currentPage, hasMore, fetchProducts, loadMoreProducts } = useProductStore();
  ...
  }, [products.length, categories.length, manufacturers.length, fetchProducts, fetchCategories, fetchManufacturers]);

  //  set initial category from URL

  useEffect(() => {
    if (products.length === 0) {
      fetchProducts(1, 30, false);
    }
      ...
      <div className="max-w-7xl my-6 mx-auto">
        <div className="max-w-7xl mx-auto bg-gray-50 sticky top-30 z-10 ">
          ...
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-300 mb-2">
            <div className={`grid gap-4 items-center ${filters.category || filters.brand ? "grid-cols-2 md:grid-cols-4" : "grid-cols-2 md:grid-cols-3"}`}>
              {/* Search - hidden on mobile */}
              <div className="relative w-full hidden md:block">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                <input type="text" placeholder="Search products, codes, or brands..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2  bg-white border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
              ...
              <div className="relative w-full border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-blue-500">
                <select value={filters.brand} onChange={(e) => handleFilterChange("brand", e.target.value)} className="w-full px-4 py-2 bg-white rounded-lg focus:outline-none">
              ...
              {(filters.category || filters.brand) && (
                <button onClick={clearFilters} className="text-blue-600 hover:text-blue-800 font-medium text-sm sm:text-base w-full sm:w-auto text-left sm:text-left">
                  Clear all filters
                </button>
              )}
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 mt-10">
            <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-4">
              {filteredProducts.map((product) => (
                <ProductCardMain key={product.id} product={product} showDiscount={parseFloat(product.actual_price) > parseFloat(product.sell_price)} />
              ))}
            </div>
            {hasMore && filteredProducts.length > 0 && (
              <div className="flex justify-center py-10">
                <button onClick={loadMoreProducts} disabled={loadingMore} className="px-8 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-semibold rounded-lg transition-all flex items-center gap-2">
                  {loadingMore ? (
                    <>
...
function ProductCardMain({ product, showDiscount }) {
  ...
  return (
    <div className="flex flex-col sm-h-[250px] h-full min-h-[340px] bg-gray-50 rounded-lg shadow-md hover:shadow-2xl hover:scale-105 transition-transform duration-300 p-2 sm:p-3 lg:p-4">
      ...
      {!product.has_variations && product.stock_quantity > 0 && product.available_quantity > 0 && (
        <>
          <div className="mt-auto w-full">
