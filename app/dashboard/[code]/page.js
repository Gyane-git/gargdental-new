...
const getProductByCode = async (code) => {
  try {
    ...
    return {
      ...
      // Our API only ever returns image URLs under `files_full_url` - it never has
      // gallery/images/product_images keys, so those always resolved to a literal `[]` here,
      // and since `[] || x` short-circuits (empty arrays are truthy in JS), every consumer's
      // fallback chain (`product.gallery || ... || product.files_full_url`) stopped at that
      // empty array and never reached the real URLs. Populate all aliases from the one real
      // source field instead of fabricating empty arrays.
      gallery: product.files_full_url || [],
      images: product.files_full_url || [],
      product_images: product.files_full_url || [],
