...
export default function ManageMyAccount({ onEditProfile, user, homeAddress, defaultBillingAddress, onEditAddress, onChangePassword, onRemoveAccount, provinces = [], cities = [], zones = [] }) {
  // console.log("homeAddress", homeAddress);
  // console.log("defaultBillingAddress", defaultBillingAddress);
  // console.log("user", user);
  const provinceName = provinces.find((p) => p.id === homeAddress?.province_id)?.name || "";
  const cityName = cities.find((c) => c.id === homeAddress?.city_id)?.name || "";
  const zoneName = zones.find((z) => z.id === homeAddress?.zone_id)?.zone_name || "";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-gray-50 rounded-xl shadow p-4 hover:shadow-lg transition-shadow duration-300">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={onEditProfile} className="text-blue-500 text-sm underline cursor-pointer">
                EDIT
              </button>
            </div>
            <div className="text-gray-700 text-sm">{user.full_name || `${user.firstName || ""} ${user.lastName || ""}`.trim()}</div>
            <div className="text-gray-700 text-sm max-w-[50px]">{user.email}</div>
          ...
          <div className="bg-gray-50 rounded-xl shadow p-4">
            <div className="flex justify-between items-center mb-2">
              ...
              <button onClick={onEditAddress} className="text-blue-500 text-sm underline cursor-pointer">
                EDIT
              </button>
            </div>
            <div className="text-gray-700 text-sm">{homeAddress?.full_name}</div>
            <div className="text-gray-700 text-sm">
              {provinceName} - {cityName} - {zoneName}
            </div>
            <div className="text-gray-700 text-sm">{homeAddress?.phone}</div>
            <div className="text-gray-500 text-sm">{homeAddress?.address_type === "H" ? "Home" : homeAddress?.address_type === "O" ? "Office" : ""} Address</div>
          </div>

          {/* Billing Address */}
          <div className="bg-gray-50 rounded-xl shadow p-4">
            <div className="font-semibold mb-2">Default Billing Address</div>
            <div className="text-gray-700 text-sm">{defaultBillingAddress?.full_name}</div>
            <div className="text-gray-700 text-sm">
              {provinceName} - {cityName} - {zoneName}
            </div>
            <div className="text-gray-700 text-sm">{defaultBillingAddress?.phone}</div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-xl shadow p-4 md:p-6 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              ...
              <button onClick={onChangePassword} className="text-blue-600 hover:text-blue-800 text-sm font-medium cursor-pointer">
                Change
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              ...
              <button onClick={handleLogout} className="text-red-600 hover:text-red-800 text-sm font-medium cursor-pointer">
                Logout
              </button>
            </div>
            ...
            <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
              ...
              <button onClick={onRemoveAccount} className="text-red-600 hover:text-red-800 text-sm font-medium cursor-pointer">
                Remove
              </button>
            </div>
          </div>
